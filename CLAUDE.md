...
1. **케어 푸시 알림** — Vercel cron + Web Push API, D-day 당일 알림 (api/push-cron.js 기존 코드 있음, 연결만 필요)
2. **소셜 UI 개선** — 커뮤니티 피드 시각적 완성도
3. **마이페이지 UI 개선** — 펫 프로필 카드, 설정 섹션 정리
4. **병원 후기 탭 접근성** — care.html에서 hospital/index 진입 경로 개선
5. **AI 조언 고도화** — 건강기록·체중 추이 데이터도 프롬프트에 포함
